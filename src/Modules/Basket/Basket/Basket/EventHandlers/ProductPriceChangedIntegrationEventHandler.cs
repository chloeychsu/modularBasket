...
public class ProductPriceChangedIntegrationEventHandler(ISender sender, ILogger<ProductPriceChangedIntegrationEventHandler> logger) : IConsumer<ProductPriceChangedIntegrationEvent>
{
    ...
    {
        logger.LogInformation($"Integration Event handler: {context.Message.GetType().Name}");
        ...
        if (!result.IsSuccess)
            logger.LogError($"Error updating price in basket for product id: {context.Message.ProductId}");

        logger.LogError($"Price for product id: {context.Message.ProductId} updated in basket");
    }
}
